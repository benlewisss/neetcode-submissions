    ...
    def __init__(self, matrix: List[List[int]]):
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                north_val = (self.summed_area_table[i - 1][j] if i >= 1 else 0)
                west_val = (self.summed_area_table[i][j - 1] if j >= 1 else 0)
                north_west_val = (self.summed_area_table[i - 1][j - 1] if (i >= 1 and j >= 1) else 0)
                self.summed_area_table[i][j] = matrix[i][j] + north_val + west_val - north_west_val
